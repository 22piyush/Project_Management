...
export const submitProposal = asyncHandler(async (req, res, next) => {
    ...
    if (existingProject && existingProject.status !== "rejected") {
        return next(
            new ErrorHandler(
                "You already have an active project. Ypu can only submit new proposal if previous project is rejected.",
                400,
            ),
        );
    }
...
export const getDashboardState = asyncHandler(async (req, res, next) => {
    ...
    const now = new Date();
    const upcomingDeadlines = await Project.find({ student: studentId, deadline: { $gte: now } })
